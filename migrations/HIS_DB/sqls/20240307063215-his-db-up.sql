...
CREATE TABLE
    IF NOT EXISTS users.user (
        ...
        user_role INTEGER REFERENCES users.user_role (id) ON UPDATE CASCADE ON DELETE CASCADE,
        job INTEGER REFERENCES users.job (id) ON UPDATE CASCADE ON DELETE CASCADE,
        user_config JSON,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
...
CREATE TABLE
    IF NOT EXISTS users.qualification (
        ...
        username VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS patients.personal (
        ...
        dob VARCHAR(12),
        gender VARCHAR(10),
        payment_category INTEGER REFERENCES finance.payment_category (id) ON UPDATE CASCADE ON DELETE CASCADE,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
...
CREATE TABLE
    IF NOT EXISTS patients.clinical (
        ind SERIAL PRIMARY KEY,
        mrn VARCHAR(100) REFERENCES patients.personal (mrn) ON UPDATE CASCADE ON DELETE CASCADE,
        dm INTEGER REFERENCES patients.diabetes_description (id) ON UPDATE CASCADE ON DELETE CASCADE,
        htn INTEGER REFERENCES patients.hypertension_description (id) ON UPDATE CASCADE ON DELETE CASCADE,
        smoking BOOLEAN,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE
    IF NOT EXISTS patients.intervention (
        ind SERIAL PRIMARY KEY,
        mrn VARCHAR(50) REFERENCES patients.personal (mrn) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS radiology.study (
        ...
        arabic_name VARCHAR,
        price FLOAT,
        study_preparation VARCHAR(50) REFERENCES radiology.study_preparation (id) ON UPDATE CASCADE ON DELETE CASCADE,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
...
CREATE TABLE
    IF NOT EXISTS radiology.order (
        ...
        mrn VARCHAR(50) REFERENCES patients.personal (mrn) ON UPDATE CASCADE ON DELETE CASCADE,
        age FLOAT,
        study_id VARCHAR(150) REFERENCES radiology.study (id) ON UPDATE CASCADE ON DELETE CASCADE,
        technician VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        radiologist VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS laboratory.external_lab_test (
        ind SERIAL PRIMARY KEY,
        test_id VARCHAR(150) REFERENCES laboratory.test (id) ON UPDATE CASCADE ON DELETE CASCADE,
        external_lab_id INTEGER REFERENCES laboratory.external_lab (id) ON UPDATE CASCADE ON DELETE CASCADE,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
...
CREATE TABLE
    IF NOT EXISTS laboratory.order (
        ...
        end_time TIME,
        payment_category INTEGER REFERENCES finance.payment_category (id) ON UPDATE CASCADE ON DELETE CASCADE,
        test_location INTEGER REFERENCES laboratory.external_lab (id) ON UPDATE CASCADE ON DELETE CASCADE,
        chemist VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        pathologist VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        referring_phys VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        report_status VARCHAR(50),
        critical VARCHAR(50),
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS procedures.lab (
        ind SERIAL PRIMARY KEY,
        procedure_id VARCHAR(50) REFERENCES procedures.procedure (id) ON UPDATE CASCADE ON DELETE CASCADE,
        test_id VARCHAR(150) REFERENCES laboratory.test (id) ON UPDATE CASCADE ON DELETE CASCADE,
        max_limit FLOAT,
        min_limit FLOAT,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS inventory.store (
        ...
        trustee VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        last_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
...
CREATE TABLE
    IF NOT EXISTS inventory.material (
        ...
        category VARCHAR(50) REFERENCES inventory.store (id) ON UPDATE CASCADE ON DELETE CASCADE,
        store_id VARCHAR(50) REFERENCES inventory.store (id) ON UPDATE CASCADE ON DELETE CASCADE,
        price FLOAT,
        stock INTEGER,
        created VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
        updated_by VARCHAR(50) REFERENCES users.user (username) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE
    IF NOT EXISTS inventory.item_movement (
        ind SERIAL,
        id VARCHAR(50) PRIMARY KEY,
        trasnaction_id VARCHAR(50) REFERENCES inventory.transaction ON UPDATE CASCADE ON DELETE CASCADE,
        item_id VARCHAR(50),
        amount INTEGER,
