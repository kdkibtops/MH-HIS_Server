...
CREATE TRIGGER IF NOT EXISTS syncDB AFTER INSERT on study 
FOR EACH ROW 
BEGIN
...
CREATE TRIGGER IF NOT EXISTS sync_DB AFTER INSERT on series 
BEGIN
  UPDATE change_log SET proccessed = false WHERE row_id = New.referenceId;
END;

-- if updated
CREATE TRIGGER IF NOT EXISTS syncDBupdate 
...
BEGIN
  ...
  SET change_type = 'UPDATE',
      processed = FALSE
  WHERE row_id = NEW.reference_id;
END;

CREATE TRIGGER IF NOT EXISTS sync_DBupdate AFTER UPDATE on series 
BEGIN
  UPDATE change_log SET proccessed = false WHERE row_id = New.referenceId;
END;
...
CONSTRAINT fkey_patients FOREIGN KEY ("updated_by") REFERENCES "main_users" ("username") ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "main_users" (
  ...
  "uploads" json,
  "last_update" VARCHAR(75),
  CONSTRAINT fkey_users FOREIGN KEY ("user_role") REFERENCES "main_user_roles" ("role_id") ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "main_studies" (
  ...
  "updated_by" varchar(50),
  "last_update" VARCHAR(75),
CONSTRAINT fkey_studies FOREIGN KEY ("updated_by") REFERENCES "main_users" ("username") ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "main_orders" (
  ...
  "updated_by" varchar(50),
  "last_update" VARCHAR(75),
CONSTRAINT fkey_orders_updated_by FOREIGN KEY ("updated_by") REFERENCES "main_users" ("username") ON UPDATE CASCADE,
CONSTRAINT fkey_orders_mrn FOREIGN KEY ("mrn") REFERENCES "main_patients" ("mrn" )ON UPDATE CASCADE,
CONSTRAINT fkey_orders_study FOREIGN KEY ("study") REFERENCES "main_studies" ("study_id") ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "main_clinics" (
  ...
  "updated_by" varchar(50),
  "last_update" VARCHAR(75),
CONSTRAINT fkey_clinics_attending_physician FOREIGN KEY ("attending_physician") REFERENCES "main_users" ("username") ON UPDATE CASCADE,
CONSTRAINT fkey_clinics_updated_by FOREIGN KEY ("updated_by") REFERENCES "main_users" ("username") ON UPDATE CASCADE
);
